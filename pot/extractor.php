<?php
// $Id: extractor.php,v 1.18.2.6 2007/01/16 11:41:26 goba Exp $
...
*/
  ...
  function build_files(&$strings, &$file_versions, $forcename = NULL) {
    foreach ($strings as $str => $fileinfo) {
      ...
      // If occured more than once, store in general.pot
      else {
  ...
  */
  ...
  // Grab language names from the Drupal source
  ...
  function find_info_file_strings($file, &$strings) {
    ...
    foreach ($info as $key => $item) {
      // Skip the special VERSION value and version numbers
      if ($key == 'version') {
        continue;
      }
      $strings[$item][$file][] = 0;
  ...
  function explore_dir($path = '') {
    $files = glob("$path*.{php,inc,module,engine,theme,install,info,profile}", GLOB_BRACE);
